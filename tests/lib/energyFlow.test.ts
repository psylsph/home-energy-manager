...
// ---------------------------------------------------------------------------
// EV-charger home node net/gross (issue #188). The reporter's scenario:
// night, no solar, full battery, EV charging from the grid. The fix
// keeps the home-centred spoke topology intact (the energy still flows
// through the home hub, the same way solar and battery spokes do) and
// keeps the import spoke at the full grid inflow wattage. The EV
// itself is drawn as a `home → ev` spoke at the full `evcPower`, not
// a direct `grid → ev` spoke. The home node carries a sub-label with
// the gross consumption (net + EV draw) so the user can reconcile
// the spokes (12.3 kW import, 7.1 kW EV, 5.2 kW net) against the
// total without having to do the arithmetic themselves.
// ---------------------------------------------------------------------------
describe('buildEnergyFlows — EV charger net/gross home node (issue #188)', () => {
  it("reporter's exact scenario: 12.3 kW import, 7.1 kW EV, 5.2 kW home, idle battery", () => {
    // Night, no solar, full battery (idle, not charging), EV drawing
    // 7.1 kW, 5.2 kW house load, 12.3 kW total import. The import
    // spoke is drawn at the FULL grid inflow (12.3 kW), the EV
    // spoke at the full EV draw (7.1 kW), and the home node shows
    // 5.2 kW (net) with a "Gross 12.3kW" sub-label.
    const vm = buildEnergyFlows(
      snap({
        grid_power: -12300,
        home_power: 5200,
      ...
      { evcPowerW: 7100, showEvc: true },
    );
    const importFlow = flowById(vm, 'import');
    expect(importFlow).toBeDefined();
    expect(importFlow!.watts).toBe(12300);
    const ev = flowById(vm, 'ev');
    expect(ev).toBeDefined();
    expect(ev!.watts).toBe(7100);
    expect(ev!.from).toBe('home');
    expect(ev!.to).toBe('ev');
    // No direct `grid_ev` spoke.
    expect(flowById(vm, 'grid_ev')).toBeUndefined();
  });

  it('import spoke is red, ev spoke is blue (home-as-source colour, issue #170 priority)', () => {
    const vm = buildEnergyFlows(
      snap({ grid_power: -12300, home_power: 5200, battery_state: 'idle', battery_power: 0 }),
      { evcPowerW: 7100, showEvc: true },
    );
    expect(flowById(vm, 'import')!.color).toBe(FLOW_COLORS.grid);
    // home → ev, no special rules fire, so the spoke colour is the
    // home identity (blue).
    expect(flowById(vm, 'ev')!.color).toBe(FLOW_COLORS.home);
  });

  it('import spoke is the full grid inflow (minus grid_charge) when EV is also drawing', () => {
    // 1.5 kW battery charging + 1 kW EV + 0.5 kW home = 3 kW import.
    const vm = buildEnergyFlows(
      snap({
        grid_power: -3000,
        home_power: 500,
      ...
      { evcPowerW: 1000, showEvc: true },
    );
    // import is the full grid inflow minus the grid_charge attribution
    // (3 - 1.5 = 1.5 kW). The EV is NOT subtracted from the import.
    expect(flowById(vm, 'import')!.watts).toBe(1500);
    expect(flowById(vm, 'grid_charge')!.watts).toBe(1500);
    expect(flowById(vm, 'ev')!.watts).toBe(1000);
  });

  it('battery claim consumes the whole import: import is zero, EV still drawn from home', () => {
    // 2 kW import, 2 kW battery charge, 0.5 kW home, 1 kW EV. Battery
    // claim absorbs the entire import, so the import spoke is gone.
    // The EV is still drawn from the home (not the grid).
    const vm = buildEnergyFlows(
      snap({
        grid_power: -2000,
        home_power: 500,
    ...
    expect(flowById(vm, 'grid_charge')!.watts).toBe(2000);
    expect(flowById(vm, 'import')).toBeUndefined();
    const ev = flowById(vm, 'ev');
    expect(ev).toBeDefined();
    expect(ev!.watts).toBe(1000);
    expect(ev!.from).toBe('home');
  });

  it('EV drawing while grid is exporting: import is zero, EV drawn from home', () => {
    // 2 kW solar, 1 kW export, 1.5 kW EV, 0.5 kW home. Grid is
    // exporting — no import spoke. The EV is still drawn as
    // `home → ev`.
    const vm = buildEnergyFlows(
      snap({ solar_power: 2000, grid_power: 1000, home_power: 500, battery_state: 'idle', battery_power: 0 }),
      { evcPowerW: 1500, showEvc: true },
    );
    expect(flowById(vm, 'import')).toBeUndefined();
    const ev = flowById(vm, 'ev');
    expect(ev).toBeDefined();
    expect(ev!.watts).toBe(1500);
    expect(ev!.from).toBe('home');
  });

  it('EV idle (power at zero) does not emit the ev spoke', () => {
    const vm = buildEnergyFlows(
      snap({ grid_power: -1000, home_power: 1000, battery_state: 'idle', battery_power: 0 }),
      { evcPowerW: 0, showEvc: true },
    );
    expect(flowById(vm, 'ev')).toBeUndefined();
    // import is the only grid-sourced spoke.
    expect(flowById(vm, 'import')!.watts).toBe(1000);
  });

  it('showEvc=false leaves the diagram unchanged from the no-EV case', () => {
    const vm = buildEnergyFlows(
      snap({ grid_power: -5000, home_power: 5000, battery_state: 'idle', battery_power: 0 }),
      { evcPowerW: 3000, showEvc: false },
    );
    expect(flowById(vm, 'ev')).toBeUndefined();
    expect(flowById(vm, 'import')!.watts).toBe(5000);
  });

  it('home node carries a gross sub-label when an EV is configured and drawing', () => {
    // 5.2 kW net + 7.1 kW EV = 12.3 kW gross.
    const vm = buildEnergyFlows(
      snap({ grid_power: -12300, home_power: 5200, battery_state: 'idle', battery_power: 0 }),
      { evcPowerW: 7100, showEvc: true },
    );
    const home = vm.nodes.find((n) => n.id === 'home')!;
    expect(home.value).toBe('5.2kW');
    expect(home.unit).toBe('Net');
    expect(home.subLabel).toBe('Gross 12.3kW');
  });

  it('home node has no gross sub-label when no EV is configured', () => {
    const vm = buildEnergyFlows(
      snap({ home_power: 5000 }),
    );
    const home = vm.nodes.find((n) => n.id === 'home')!;
    expect(home.unit).toBe('Net');
    expect(home.subLabel).toBeUndefined();
  });

  it('home node has no gross sub-label when EV is configured but idle', () => {
    // EV charger configured and reachable, but not drawing.
    const vm = buildEnergyFlows(
      snap({ home_power: 1000 }),
      { evcPowerW: 0, showEvc: true },
    );
    const home = vm.nodes.find((n) => n.id === 'home')!;
    expect(home.subLabel).toBeUndefined();
  ...
  it('home node has no gross sub-label when busbar is below noise', () => {
    // Busbar reading is 5 W (below default 20 W noise floor). The
    // gross sub-label would be misleading (5 W + EV doesn't reflect
    // any real house consumption), so it's suppressed.
    const vm = buildEnergyFlows(
      snap({ home_power: 5, battery_state: 'idle', battery_power: 0 }),
