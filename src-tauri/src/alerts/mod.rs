...
/// Notification metadata for an ntfy attachment upload. Grouping these keeps
/// [`send_ntfy_attachment`] under clippy's argument-count limit and makes call
/// sites self-documenting.
#[derive(Debug, Clone, Copy)]
pub struct NtfyMessage<'a> {
    /// Notification title (ntfy `Title` header).
    pub title: &'a str,
    /// Notification body text (ntfy `Message` header) shown on the card.
    /// Sanitised to a single line before sending — see [`sanitize_header_value`].
    pub message: &'a str,
    /// Comma-separated emoji shortcodes (ntfy `Tags` header).
    pub tags: &'a str,
    /// Optional `Click` URL (ntfy `Click` header) making the notification card
    /// open this URL when tapped. Used to deep-link a support bundle to its
    /// GitHub issue. `None` omits the header.
    pub click: Option<&'a str>,
}

/// Collapse a header value to a single line of legal HTTP header characters.
///
/// The `http` crate's `HeaderValue::from_str` rejects the bytes that can't
/// appear in an RFC 7230 header field value — crucially CR (`\r`) and LF
/// (`\n`), which a multi-line message body naturally contains, plus the rest
/// of the C0 control range. Passing such a value straight to `.header()`
/// surfaces as `http: failed to parse header value` at request time, which is
/// exactly what killed the first support-bundle submission (the `Message`
/// header carried a multi-line summary). This helper is the defence: any
/// disallowed byte becomes a space, and runs of spaces collapse to one. Tabs
/// are preserved (they're legal header bytes). Applied to every header in
/// [`send_ntfy_attachment`] that can carry caller-derived (user) text.
fn sanitize_header_value(value: &str) -> String {
    let mut out = String::with_capacity(value.len());
    let mut prev_space = false;
    for c in value.chars() {
        // Legal header-value bytes per http::HeaderValue: HTAB, plus the
        // visible ASCII range 0x20-0x7E, plus obs-text 0x80-0xFF. Everything
        // else (CR, LF, NUL, other C0 controls, DEL) collapses to a space.
        let is_legal = c == '\t' || (c as u32 >= 0x20 && c as u32 != 0x7f);
        if is_legal {
            if c == ' ' {
                if !prev_space {
                    out.push(' ');
            ...
            // Replace the illegal byte with a single space, collapsing runs.
            out.push(' ');
            prev_space = true;
        }
    }
...
/// Upload a file as an ntfy.sh (or self-hosted ntfy) attachment.
///
/// Unlike [`send_ntfy_message`] (which carries only a text body), this PUTs
/// the raw file bytes to `{server}/{topic}` and passes the notification
/// metadata — title, message, filename, tags, optional click URL — via ntfy's
/// HTTP headers. ntfy stores the body as a downloadable attachment and renders
/// the header fields as the notification card. This is the delivery path for
/// the automated support bundle (#125): the bundle is too large for ntfy's
/// inline message body (capped at ~16 KB), so it has to ship as an attachment.
///
/// `content_type` should match the attachment (e.g. `application/json`).
/// Header values are sanitised via [`sanitize_header_value`] before sending,
/// so a multi-line `message` or a user-supplied click URL containing illegal
/// bytes cannot break the request.
///
/// Uses `ureq` (synchronous) — call from `tokio::task::spawn_blocking`.
pub fn send_ntfy_attachment(
    topic: &str,
    server: &str,
    filename: &str,
    content_type: &str,
    ...
    let url = format!("{}/{}", server.trim_end_matches('/'), topic);
    let server_display = server.to_string();
    let title = sanitize_header_value(msg.title);
    let message = sanitize_header_value(msg.message);
    let tags = sanitize_header_value(msg.tags);
    let filename_clean = sanitize_header_value(filename);
    let mut req = ureq::put(&url)
        .header("Filename", &filename_clean)
        .header("Title", &title)
        .header("Message", &message)
        .header("Tags", &tags)
        .header("Content-Type", content_type);
    if let Some(click) = msg.click {
        let click_clean = sanitize_header_value(click);
        if !click_clean.is_empty() {
            req = req.header("Click", &click_clean);
        }
    }
...
mod tests {
    ...
    // --- ntfy header sanitisation (root-cause fix for the #125 attachment
    // failure: a multi-line Message header produced
    // `http: failed to parse header value`). ---

    #[test]
    fn sanitize_collapses_newlines_to_single_line() {
        let multi = "HEM Support Bundle\nID: hem-X\nCategory: battery";
        let out = super::sanitize_header_value(multi);
        assert!(!out.contains('\n'), "still had a newline: {out:?}");
        assert!(!out.contains('\r'));
        assert_eq!(out, "HEM Support Bundle ID: hem-X Category: battery");
    }

    #[test]
    fn sanitize_strips_crlf_and_control_bytes() {
        let dirty = "line1\r\nline2\tnul\0del\x7f";
        let out = super::sanitize_header_value(dirty);
        assert!(out.contains('\t'), "tab should be preserved");
        for c in out.chars() {
            assert!(
                c == '\t' || (c as u32 >= 0x20 && c as u32 != 0x7f),
                "illegal byte survived: {out:?}"
            );
        }
    }
    ...
    fn sanitize_collapses_runs_of_spaces() {
        let out = super::sanitize_header_value("a   b\n  c");
        assert_eq!(out, "a b c");
    }
