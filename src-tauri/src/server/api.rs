...
pub async fn update_settings(
    ...
    let save_result = crate::settings::Settings::try_update_async(move |persist| {
        ...
        // credential) so the caller can deliver it after the commit. We
        // must clone here because the closure's `persist` is dropped when
        // the lock is released at the end of Settings::update.
        Ok((persist.clone(), generated_secret))
    })
    .await;
    // Outer Err = lock/save failure (500); inner Err = validation
    // rejection (400, nothing persisted); Ok = saved, snapshot returned.
    let (persist_for_log, generated_secret) = match save_result {
        Err(e) => return server_error(&format!("Failed to save settings: {e}")),
        Ok(Err(msg)) => return error_response(&msg),
        Ok(Ok((snapshot, generated))) => (snapshot, generated),
    };
