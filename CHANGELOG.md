...
- **Windows MSIX release asset** — Windows releases now include both the
  existing MSI installer and a generated MSIX package, plus a `.msixupload`
  archive for Microsoft Store submission. MSIX identity and publisher fields
  can be supplied via GitHub Actions repository variables.
...
- **Power PDF renamed to Consumption Report** — The rich Power-page report is
  now labelled as a Consumption Report while keeping the full summary layout:
  KPI cards, combined power flow, solar/home, grid import/export, battery
  activity, breakdown charts, highlights, and bucket table.
- **Clearer export confirmations** — Power and History CSV exports now state
  that the file has been downloaded to the Downloads folder instead of implying
