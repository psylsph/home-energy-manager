...
- **Always work on `master`.** There is a single developer on this project, so feature branches and worktrees add ceremony without benefit — commit and push directly to `master`, and tag releases on `master` as the release process describes. PRs are still how Dependabot dependency bumps and outside contributions arrive; merge those into `master` as normal.
