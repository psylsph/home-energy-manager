...
export default function SettingsPage() {
  ...
  // Keys are generated by HEM and shown once; the secret never round-trips
  // through GET /api/settings, so there is no key input field to hydrate.
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // GET /api/settings never returns the secret — only metadata
  ...
  // Rotate the authenticated API credential: HEM generates the secret and
  // returns it exactly once. The backend stores only a verifier; the UI
  // shows the secret with a copy action and warns it is never shown again.
  // Include the port so the long-standing "save key + port" behaviour is
  // preserved for local administrators.
  const handleApiKeyGenerate = async () => {
    ...
    try {
      const response = await apiPost<{ data?: { api_key?: string } }>('/api/settings', {
        api_key_generate: true,
        api_port: apiPort,
      });
      const secret = response.data?.api_key ?? '';
      if (!secret) {
        setMessage({ text: 'Key generation did not return a secret', ok: false });
        return;
      }
      setGeneratedApiKey(secret);
      setApiKeyConfigured(true);
      setApiKeyLast4(secret.slice(-4));
      setMessage({
        text: 'New API key generated. Copy it now — it will not be shown again. Key changes apply immediately to a running API server; starting it or changing its port requires an app restart.',
        ok: true,
      });
    } catch (error) {
      setMessage({
        text: error instanceof Error ? error.message : 'Failed to generate API key',
        ok: false,
      });
    } finally {
  ...
  // Clear the configured credential entirely (verifier included).
  const handleApiKeyClear = async () => {
    ...
    setApiKeySaving(true);
    try {
      await apiPost('/api/settings', { api_key: '', api_port: apiPort });
      setGeneratedApiKey(null);
      setApiKeyConfigured(false);
      setApiKeyLast4('');
      setMessage({ text: 'API key cleared. Any saved key changes apply immediately to a running API server; starting it requires an app restart.', ok: true });
    } catch (error) {
      setMessage({
      ...
      <section className="bg-bg-surface rounded-xl p-5 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-3 pt-3 border-t border-bg-elevated">
          ...
          <p className="text-text-secondary text-xs font-sans">
            ...
            These settings can only be changed from this machine.
          </p>
          <p className="text-text-secondary text-xs font-sans">
            {apiKeyConfigured
              ? `Key configured${apiKeyLast4 ? ` (ends ${apiKeyLast4})` : ''}. Generating a new key replaces the old one immediately.`
              : 'No key configured — the authenticated API is off.'}
          </p>
          {generatedApiKey && (
            <div className="flex flex-col gap-1 bg-bg-elevated rounded-lg p-3">
              <span className="text-text-primary text-xs font-sans font-semibold">
                New API key — copy it now, it will not be shown again:
              </span>
              <div className="flex items-center gap-3">
                <code className="text-accent text-sm font-mono flex-1 min-w-0 break-all select-all">{generatedApiKey}</code>
                <button
                  onClick={() => { void navigator.clipboard.writeText(generatedApiKey); }}
                  className="bg-bg-base text-text-primary font-sans text-sm px-4 py-2 rounded-lg hover:opacity-90 transition-opacity shrink-0"
                >Copy</button>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => { void handleApiKeyGenerate(); }}
              disabled={apiKeySaving}
              className="self-start bg-accent text-on-accent font-sans font-semibold text-sm px-5 py-2 rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              {apiKeySaving ? 'Generating…' : apiKeyConfigured ? 'Generate new key' : 'Generate API key'}
            </button>
            {apiKeyConfigured && (
              <button
